...
func TestHTMLStandaloneEscapedAccessibleAndDeterministic(t *testing.T) {
	...
	if got := fmt.Sprintf("%x", sha256.Sum256(first.Bytes())); got != "0dbe0212178dbd991f77e3aa4c058aa7414e6371fe88a6e71694a292cc95ca03" {
		t.Fatalf("HTML golden hash = %s", got)
	}
	output := first.String()
	const footerBrand = `Generated by CredScope · Bavlik · <a href="https://abdullahcv.com">abdullahcv.com</a>`
	for _, expected := range []string{"<!doctype html>", "Content-Security-Policy", "<header>", "<main>", "<footer>", "Rotate safely", "Typed graph", "Risk score", "Evidence confidence", "&lt;script&gt;", footerBrand} {
		if !strings.Contains(output, expected) {
			t.Errorf("missing %q", expected)
		}
	}
	if strings.Count(output, "Bavlik") != 1 {
		t.Fatalf("branding must appear exactly once in the footer, found %d", strings.Count(output, "Bavlik"))
	}
	if strings.Count(output, `href="https://abdullahcv.com"`) != 1 {
		t.Fatal("website href must appear exactly once and be exactly https://abdullahcv.com")
	}
	withoutFooterLink := strings.Replace(output, footerBrand, "", 1)
	if strings.Contains(withoutFooterLink, "https://") || strings.Contains(withoutFooterLink, "http://") || strings.Count(output, "</style>") != 1 || strings.Contains(output, "RAW_SECRET_NOT_IN_MODEL") || strings.Contains(output, "<script") {
