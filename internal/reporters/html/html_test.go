...
func TestHTMLStandaloneEscapedAccessibleAndDeterministic(t *testing.T) {
	...
	if got := fmt.Sprintf("%x", sha256.Sum256(first.Bytes())); got != "40a047d41f60edafcc6197641c8ea3e5301e493f7bcf86bd87457518b79ad845" {
		t.Fatalf("HTML golden hash = %s", got)
	}
	output := first.String()
	for _, expected := range []string{"<!doctype html>", "Content-Security-Policy", "<header>", "<main>", "<footer>", "Rotate safely", "Typed graph", "Risk score", "Evidence confidence", "&lt;script&gt;", "Generated by CredScope · Bavlik"} {
